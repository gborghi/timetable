...
\grokfig{fig_vincoli_leoni_gattini}{Leoni HARD e gattini SOFT
sui due piatti della bilancia che $\pi$Tantum tiene in mano.}
...
\begin{schemabox}[title={Quadro di riferimento rapido}]
\textbf{I cinque stati} (dal pi\`u permissivo al pi\`u rigido):
\begin{itemize}\setlength\itemsep{0pt}
  \item \textcolor{cAllow}{\textbf{ALLOWED}} --- neutro, nessun
        effetto (default).
  \item \textcolor{cSoft}{\textbf{SOFT}} --- ``preferirei di no'':
        penalit\`a se violato, ma ammesso.
  \item \textcolor{cPref}{\textbf{PREFERRED}} --- ``preferirei di
        s\`i'': bonus se soddisfatto.
  \item \textcolor{cHard}{\textbf{HARD}} --- divieto assoluto: la
        soluzione che lo viola \`e scartata.
  \item \textcolor{cEnf}{\textbf{ENFORCED}} --- obbligo assoluto,
        opposto di HARD.
\end{itemize}
\textbf{Le tre forme speciali} di cattedra:
\begin{itemize}\setlength\itemsep{0pt}
  \item \emph{compresenza} --- pi\`u docenti sulla stessa
        (classe, materia) nella stessa ora;
  \item \emph{sostegno} --- docente ``ombra'' agganciato a una
        classe;
  \item \emph{potenziamento} --- ore senza una classe fissa.
\end{itemize}
\textbf{Un vincolo scritto nel DSL}, come esempio:\\
\texttt{forall t in Docenti: count(t, Sabato) = 0}\\
(``nessun docente lavora il sabato''). La grammatica completa \`e
nella \hyperref[part:dsl]{Parte sul DSL}.
\end{schemabox}

\section{La tassonomia dei cinque stati}
\label{sec:cinque-stati}
...
\grokfig{fig_free_day_or_cancello}{Il cancelletto a due porte:
giorno libero oppure mezza giornata, OR esclusivo.}

\grokfig{fig_soft_podio_peluche}{Il podio dei vincoli SOFT con
tre peluche premiati: no buchi, distribuzione uniforme, no sesta
ora.}

\section{Le preferenze di giorno libero per i docenti}
\label{sec:vincoli-giorno-libero-docente}

I docenti hanno diritto, per contratto collettivo nazionale
del settore scolastico italiano, ad almeno un giorno libero
a settimana per il personale a tempo pieno. $\pi$Tantum
modella questa esigenza con un'architettura a due livelli:
un \emph{pavimento} HARD universale ``almeno N giorni
liberi'' e una preferenza \emph{ordinata in priorit\`a} a
tre livelli SOFT.

\subsection*{Pavimento HARD: ``almeno $N$ giorni liberi''}

Il vincolo HARD \`e universale ma il numero $N$ \`e
\emph{per docente}: la colonna
\texttt{Teacher.min\_free\_days} (Integer, NOT NULL, DEFAULT
1) carica il valore di $n_t$. Il sistema impone, per ogni
docente $t$,
\[
   \sum_{d \in \mathrm{giorni}} \mathbf{1}\{\text{$t$ ha almeno
   una lezione il giorno $d$}\}
   \;\le\; |\mathrm{giorni}| - n_t.
\]
Equivalentemente, $t$ deve avere almeno $n_t$ giornate con
zero lezioni. Casi tipici:
\begin{itemize}
  \item $n_t = 1$: tempo pieno CCNL (default per circa il
    70\% del corpo docente nei profili stress).
  \item $n_t = 2$: cattedra ridotta o ``orario spezzato''
    (circa 25\% nei profili stress).
  \item $n_t = 3$: incarichi part-time pesanti (circa 5\%).
  \item $n_t = 0$: vincolo disabilitato (caso eccezionale,
    tipicamente per docenti in supplenza breve).
\end{itemize}
Il pragma DSL canonico \`e
\begin{verbatim}
teacher_at_least_n_free_days("Rossi", 2)
\end{verbatim}
ed \`e registrato con \texttt{level=both} nel compilatore
DSL: si applica uniformemente a Phase A, Phase B,
\texttt{cpsat\_week}, \texttt{cpsat\_day\_skip\_phase\_a} e
\texttt{cpsat\_day\_soft\_hint}. Il vecchio meccanismo
\texttt{free\_day\_choice\_3way} viveva solo in Phase A e
veniva silenziosamente bypassato dalle ultime tre tecniche
producendo \texttt{hard\_violation}; il nuovo pragma
risolve l'asimmetria.

\paragraph{Vincolo di feasibility.} Con \texttt{max\_per\_day} = 2
e $n_t = 3$, le ore settimanali del docente devono stare
in $|\mathrm{giorni}| - n_t = 3$ giorni di lavoro: 5 ore
sono ammissibili (2+2+1), 7 ore no (max teorico 6). I
profili stress evitano la combinazione $n_t = 3$ con cattedre
oltre le 6 ore. Se l'utente imposta valori incompatibili
con il monte ore, il solver restituisce
\texttt{INFEASIBLE} -- nessun fallback silenzioso.

\subsection*{Preferenze SOFT ordinate in priorit\`a}

Sopra il pavimento, ogni docente ha tre preferenze ordinate
nella tabella \texttt{teacher\_free\_day\_preferences}
(colonne \texttt{teacher\_id}, \texttt{day} 1..6,
\texttt{priority} 1..3). Le tre priorit\`a corrispondono a
penalty fisse:
\begin{itemize}
  \item priorit\`a 1: peso 30 (prima scelta -- la pi\`u
    costosa da ignorare),
  \item priorit\`a 2: peso 20 (seconda scelta),
  \item priorit\`a 3: peso 10 (terza scelta).
\end{itemize}
La formula \`e $w_p = 40 - 10 p$. Per ogni riga il
traduttore DSL emette il pragma SOFT canonico
\begin{verbatim}
teacher_preferred_free_day_penalty("Rossi", 6, 30)
\end{verbatim}
che contribuisce al costo SOFT con $w_p \cdot
\mathbf{1}\{\text{Rossi lavora il giorno 6}\}$. Il
solver minimizza la somma, quindi tende a onorare prima la
priorit\`a 1 perch\'e ignorarla costa di pi\`u.

\subsection*{Esempi numerici per $N \in \{1, 2, 3\}$}

\paragraph{$N = 1$ (tempo pieno).} Cinque ore settimanali,
preferenze [Luned\`i pri.1, Marted\`i pri.2, Mercoled\`i
pri.3]. Il pavimento HARD richiede $\ge 1$ giorno libero.
Le tre penalty in conflitto sono $30 \cdot \mathbf{1}_{\mathrm{Lun}} + 20 \cdot \mathbf{1}_{\mathrm{Mar}} + 10 \cdot
\mathbf{1}_{\mathrm{Mer}}$; il solver paga 0 lasciando libero
Luned\`i, 30 lasciando libero Marted\`i (paga la
priorit\`a 1), e cos\`i via. La scelta razionale \`e
quindi Luned\`i, salvo conflitti con altri vincoli HARD.

\paragraph{$N = 2$ (orario spezzato).} Quattro ore
settimanali, stesse preferenze. Il pavimento HARD impone
\emph{due} giorni liberi -- il docente lavora al massimo
quattro giorni. La distribuzione ottima sotto le tre
penalty SOFT lascia liberi Luned\`i e Marted\`i (cost = 10:
solo la priorit\`a 3 paga). Le 4 ore si distribuiscono
sui 4 giorni rimanenti (Mer..Sab) -- una per giorno.

\paragraph{$N = 3$ (part-time forte).} Cinque ore
settimanali, stesse preferenze. Il pavimento HARD impone
\emph{tre} giorni liberi -- il docente lavora al massimo
tre giorni. Con \texttt{max\_per\_day} = 2 le 5 ore si
distribuiscono come $2 + 2 + 1$ su tre giorni; il solver
sceglie i tre giorni che lasciano liberi Luned\`i,
Marted\`i, Mercoled\`i (cost SOFT = 0). Se invece le ore
fossero 7, la combinazione $N = 3$ + \texttt{max\_per\_day} = 2
sarebbe \texttt{INFEASIBLE} (capacit\`a $3 \times 2 = 6 < 7$).

\subsection*{Provalo (SQL)}

Per ispezionare le preferenze del DB:
\begin{verbatim}
-- pavimento HARD per docente:
SELECT t.name, t.min_free_days
  FROM teachers t ORDER BY t.id;

-- distribuzione stratificata nei profili stress (atteso 70/25/5):
SELECT min_free_days, COUNT(*)
  FROM teachers GROUP BY min_free_days;

-- 3 preferenze SOFT priority-ordered:
...
\subsection*{Caso particolare: vincolo HARD su un giorno
specifico}

Per docenti a tempo parziale che debbano \emph{per
contratto} essere fuori servizio in un giorno specifico
(per esempio Luned\`i) il sistema mantiene la tabella
distinta \texttt{teacher\_mandatory\_free\_days}: ogni
riga \`e tradotta come HARD ``zero lezioni il giorno X''.
Il pavimento universale e le preferenze SOFT continuano a
valere; la riga HARD per-giorno semplicemente forza un
ulteriore vincolo che il solver deve rispettare.
...
disponibili. I docenti di potenziamento portano un badge
\textbf{POT} viola. Non sono per\`o i primi della lista:
l'ordine \`e disposizione ufficiale (\textbf{DISP}), poi
buco (\textbf{BUCO}), poi potenziamento, poi gli altri
liberi. Il potenziamento resta una cattedra senza
\texttt{Lesson}; le ore di disposizione, invece, sono
slot esatti (vedi sotto).

\subsection{Ore di disposizione (standby)}
\index{Disposizione}

\textbf{Caso d'uso:} ogni docente pu\`o avere una quota
settimanale di ore di tipo \emph{disposizione}. Non sono
lezioni: non hanno classe n\'e aula. Servono a massimizzare
la presenza di docenti di servizio negli slot pi\`u
critici (sabato, luned\`i prima ora, prime ore in
generale, o una griglia custom).

\textbf{Configurazione:}
\begin{enumerate}
\item Tab \texttt{/teachers}: campo \emph{Ore di
disposizione} sulla scheda del docente (quota settimanale).
\item Tab \texttt{/assenze-supplenze}, bottone \emph{Ore di
disposizione}: tetto scolastico, eleggibilit\`a
(\texttt{all} oppure \texttt{under\_contract}),
\emph{Salva e piazza}.
\end{enumerate}

\textbf{Cosa fa il sistema:} un piazzatore greedy
post-solve (\texttt{webui/backend/disposizione.py}),
\emph{non} un vincolo CP-SAT, scrive
\texttt{Lesson} sentinella con
\texttt{class\_name=\_\_disposizione\_\_} e
\texttt{subject=Disposizione}. La sentinella non entra
nel carico classe, nelle aule, in H1/H2/H3 n\'e in H\_C
(sei ore consecutive). In coverage il docente resta
disponibile (\texttt{is\_disposizione=true}). Il
piazzamento \`e indipendente dalla materia. Un tetto
scolastico scala le quote in proporzione. Dopo il
piazzamento automatico le ore si spostano a mano con
\texttt{/api/schedule/move-lesson} (viste Orario per
docente / per slot).

\textbf{Visibilit\`a nel tab Assenze/Supplenze:} badge
\textbf{DISP} azzurro sui docenti in disposizione
ufficiale; \textbf{BUCO} ambra su chi ha un buco fra
prima e ultima lezione reale del giorno; \textbf{MAT} su
chi insegna la stessa materia della lezione o del
collega assente. Filtri combinabili nel modal cella
(stessa materia, materie del collega, disp / buco /
libero / pot / sotto contratto + ricerca testuale).
...
\section{Caricare i vincoli da xlsx: il linguaggio \emph{Vincoli}}
\label{sec:vincoli-xlsx}

\begin{novitabox}
Oltre a creare i vincoli uno per uno dalla UI, si pu\`o \emph{caricarli in
blocco} da un foglio di calcolo. Un unico foglio \texttt{Vincoli} con una
colonna discriminante \texttt{tipo\_vincolo} consente a un utente non
tecnico di definire le regole pi\`u comuni con un vocabolario semplice; una
riga speciale \texttt{raw\_dsl} \`e la via di fuga per chi vuole scrivere
direttamente un'espressione DSL completa (la grammatica \`e ormai del tutto
generale, cfr.\ \cref{sec:dsl-universale}).
\end{novitabox}

L'engine resta agnostico: il \emph{parser} \texttt{\_vincoli\_parser.py}
trasforma ogni riga in un \emph{intento} di creazione --- o un'espressione
DSL da passare al path dei \texttt{GeneralConstraint}, o un inserimento ORM
diretto (matrice di indisponibilit\`a, compresenza). Ogni espressione DSL
emessa viene verificata: deve passare il \emph{parse} della grammatica reale
prima di essere salvata.

\subsection{Il foglio \texttt{Vincoli}}

Le colonne, nell'ordine: \texttt{tipo\_vincolo}, \texttt{entita},
\texttt{nome}, \texttt{materia}, \texttt{giorno}, \texttt{ora\_da},
\texttt{ora\_a}, \texttt{valore}, \texttt{livello}, \texttt{peso},
\texttt{dsl}, \texttt{note}. Il \texttt{giorno} accetta \texttt{lun..sab}
oppure \texttt{1..6} (vuoto $=$ tutti i giorni); le ore vanno da 8 a 13
incluse (vuoto $=$ intera giornata); \texttt{livello} \`e uno dei cinque
stati (\cref{sec:cinque-stati}); \texttt{peso} \`e la penalit\`a SOFT.

\begin{table}[h]
\centering\small
\begin{tabular}{@{}lll@{}}
\toprule
\textsc{tipo\_vincolo} & \textsc{target} & \textsc{significato} \\
\midrule
\texttt{indisponibilita}        & ORM & cella docente non disponibile \\
\texttt{indisp\_aula}           & ORM & cella aula non disponibile \\
\texttt{giorno\_libero}         & ORM & giorno libero HARD del docente \\
\texttt{pref\_giorno\_libero}   & ORM & giorno libero preferito (peso) \\
\texttt{compresenza}            & ORM & co-docenza (\texttt{valore} $=$ n.\ docenti) \\
\texttt{max\_ore\_giorno}       & DSL & \texttt{teacher\_max\_per\_day} \\
\texttt{max\_consecutive}       & DSL & \texttt{teacher\_max\_consecutive} \\
\texttt{no\_pomeriggio}         & DSL & \texttt{slot\_after\_hour\_penalty} \\
\texttt{no\_giorni\_consecutivi}& DSL & \texttt{no\_same\_class\_consecutive\_days} \\
\texttt{no\_buchi}              & DSL & \texttt{no\_holes\_class} \\
\texttt{presenza\_ora}          & DSL & \texttt{class\_present\_at\_hour} \\
\texttt{raw\_dsl}               & DSL & espressione DSL libera (colonna \texttt{dsl}) \\
\bottomrule
\end{tabular}
\caption{Il vocabolario \texttt{tipo\_vincolo} e la sua destinazione:
inserimento ORM diretto oppure espressione DSL generata.}
\label{tab:vincoli-vocab}
\end{table}

\begin{esempiobox}
Una riga \texttt{no\_pomeriggio} con \texttt{entita}=classe, \texttt{nome}=1A,
\texttt{valore}=14, \texttt{livello}=soft, \texttt{peso}=50 diventa
l'espressione \texttt{slot\_after\_hour\_penalty(14, 50)}; una riga
\texttt{raw\_dsl} porta la propria espressione verbatim, per esempio
\texttt{forall t in teachers: teacher\_max\_consecutive(t.name, 4)}.
\end{esempiobox}

\subsection{Endpoint e report}

Due endpoint sotto \texttt{/api/dashboard/constraints}:
\texttt{GET .../template-vincoli} produce un xlsx a tre fogli
(\texttt{Istruzioni}, \texttt{Esempi}, \texttt{Vincoli}) gi\`a documentato;
\texttt{POST .../import-vincoli} legge il file, mappa le righe e crea i
vincoli, restituendo un report con due liste distinte: \texttt{parse\_errors}
(righe malformate: \texttt{tipo} sconosciuto, giorno/ora fuori range, colonna
obbligatoria mancante, \texttt{nome} non risolto) e \texttt{create\_errors}
(intenti validi ma non creabili). Gli avvisi di compatibilit\`a del solver
(\cref{sec:dsl-constraint-compat}) completano il quadro: se una regola \`e
sintatticamente valida ma nessun solver CP la modella, l'utente legge
\emph{quale} metodo la onorer\`a (tipicamente la passata metaeuristica). La
UI espone tutto questo nella card \emph{Vincoli guidati (xlsx)} del
cruscotto.
...
Da Step 3a in avanti, le \texttt{PlessoCommutingRule} sono
traducibili interamente in clausole DSL grazie a due novit\`a
del linguaggio (cf.\ cap.~\ref{ch:dsl_generico_per_i_vincoli}):

\begin{itemize}
\item il path \texttt{l.classroom.plesso} che, data una
  lezione, risolve l'aula assegnata e poi il plesso a cui
  l'aula appartiene;
\item il predicato \texttt{consecutive(l1.slot, l2.slot)} che
  diventa vero quando le due lezioni cadono nello stesso
  giorno e differiscono di un'ora.
\end{itemize}

La regola legacy ``Centrale $\to$ Garibaldi gap minimo 1 ora,
docente Rossi'' si scrive in DSL come:
...
La helper \texttt{plesso\_commuting\_rule\_to\_dsl(rule)} in
\texttt{engine/dsl\_translator.py} produce automaticamente le
clausole DSL a partire dalla riga ORM. La suite di test
verifica zero-drift: il path DSL produce le stesse coppie di
slot proibite del path legacy
\texttt{plessi\_constraints.py}.

\section{Pattern canonici e seed legacy via DSL (Step 3b-3e)}

Step 3b-3e ha introdotto un \emph{vocabolario} di pattern
canonici riconosciuti direttamente dal compilatore DSL $\to$
CP-SAT. Ognuno di essi corrisponde a un vincolo HARD legacy che
prima viveva solo come encoding hardcoded. Ora possono essere
espressi in DSL e prodotti automaticamente dalla translation
\texttt{seed\_implicit\_hardcoded(profs)}.

Pragmas accettate:
\begin{itemize}
\item \texttt{no\_holes\_class("3B")} -- niente buchi
  nell'orario settimanale della 3B (legacy
  \texttt{add\_class\_no\_holes}).
\item \texttt{class\_present\_at\_hour("3B", 11)} -- se la 3B
  ha lezione, lo slot all'ora 11 deve essere occupato (HARD-3
  / ``uscita non prima delle 12'').
\item \texttt{class\_day\_load\_in("3B", 0, 4, 5, 6)} -- la
  3B in un giorno ha 0 ore (giorno libero) o tra 4 e 6 ore
  (HARD-1+HARD-2).
\item \texttt{teacher\_max\_per\_day("Rossi", 5)} -- Rossi non
  supera 5 ore in un giorno.
\item \texttt{cattedra\_max\_per\_day("Rossi", "3B",
  "Matematica", 2)} -- la cattedra (Rossi, 3B, Mate) ha al
  massimo 2 ore in un giorno.
\item \texttt{subject\_pair\_must("3B", "Scienzemotorie")} --
  quando la 3B ha 2 ore di motoria nello stesso giorno,
  devono essere consecutive.
\item \texttt{subject\_pair\_exists("3B", "Matematica")} --
  quando la 3B ha 2+ ore di mate nello stesso giorno, almeno
  una coppia consecutiva deve esistere.
\end{itemize}

Effetto concreto: l'utente avanzato pu\`o ora scrivere
nell'editor del modal ``+ Nuovo vincolo DSL'' un'espressione
come \texttt{no\_holes\_class("4A")} ottenendo lo stesso
risultato del toggle \texttt{hard\_no\_holes} in
\texttt{school\_classes}. Il vantaggio del DSL \`e la
componibilit\`a con \texttt{forall}, \texttt{exists},
\texttt{count}: si pu\`o restringere a sotto-insiemi di classi,
combinare con altri predicati, modificare al volo dall'UI
senza editare il database.
