...
\subsection*{Pavimento HARD: ``almeno un giorno libero''}

Il vincolo HARD \`e universale: per ogni docente $t$ il
sistema impone
\[
...
\]
dove $n_t$ \`e il valore di
\texttt{Teacher.required\_free\_days\_count} (predefinito 1,
conforme al contratto collettivo). Il pragma DSL canonico
\`e
\begin{verbatim}
teacher_at_least_n_free_days("Rossi", 1)
\end{verbatim}
...
Si consideri un docente con cinque ore settimanali e le
tre preferenze [Luned\`i pri.1, Marted\`i pri.2, Mercoled\`i
...
(p.es. una lezione fissata a Luned\`i da un altro docente
in compresenza).
...
SELECT t.name, t.required_free_days_count
