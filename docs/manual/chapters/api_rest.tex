...
\begin{itemize}
  ...
  \item \textbf{Cattedre}: \texttt{/api/assignments} +
        ...
        \texttt{bulk/\{delete,restore,lock,change-teacher,set-flag\}}
        (\texttt{restore} ricrea le cattedre da uno snapshot: \`e il
        backend dell'undo dell'eliminazione in blocco).
  ...
  \item \textbf{Coverage}: \texttt{/api/coverage/week|cell},
        \texttt{/api/coverage/disposizione}
        (\texttt{GET}/\texttt{PUT} policy scolastica;
        \texttt{POST \ldots/place} ripiazza le ore di
        disposizione sulla soluzione attiva),
        \texttt{/api/absences}, \texttt{/api/substitutions}.
        Quota per docente: \texttt{Teacher.disposizione\_hours}
        su \texttt{/api/teachers}. Le ore piazzate sono
        \texttt{Lesson} sentinella
        (\texttt{class\_name=\_\_disposizione\_\_}) e si
        spostano con \texttt{/api/schedule/move-lesson}.
  ...
  \item \textbf{Import}: \texttt{/api/import/\{entity\}}
        (con \texttt{?dry=true} per l'anteprima senza scrittura),
        \texttt{/api/import/\{entity\}/template}.
  \item \textbf{Vincoli (xlsx)}:
        \texttt{/api/dashboard/constraints/template-vincoli},
        \texttt{/api/dashboard/constraints/import-vincoli}
        (linguaggio \emph{Vincoli}, \cref{sec:vincoli-xlsx}),
        \texttt{import-file}, \texttt{export}.
\end{itemize}
