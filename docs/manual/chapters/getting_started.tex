...
\grokfig{fig_quickstart_5_passi}{I cinque passi del quick start,
dall'installazione al primo orario in circa trenta minuti.}
...
\grokfig{shot_dashboard}{Il cruscotto (Dashboard) a browser aperto:
in alto la checklist guidata dei primi passi, che si spunta da sola
...
\grokfig{shot_orario}{L'orario settimanale nella vista \emph{Per
classe}: ogni lezione \`e un blocco trascinabile; il click apre le
azioni (sposta, elimina, svincola).}
...
\grokfig{shot_optimize}{La pagina \emph{Ottimizza}: in cima il
pulsante \emph{Genera orario (consigliato)} per l'uso normale; le
schede con i parametri del solver sono nell'accordion
\emph{Modalit\`a avanzata}.}
...
e il capitolo~\ref{ch:tecniche_avanzate}
spiega quando ha senso ciascuna combinazione.

\section{Aggiungere un vincolo dalla UI}

\grokfig{shot_vincoli}{La pagina \emph{Vincoli}: da qui si aggiungono
regole (giorni liberi, indisponibilit\`a, preferenze) e si lancia il
controllo di fattibilit\`a.}
...
\grokfig{fig_import_error_pesce}{Il pesce confuso davanti a un
file di formato incompleto.}
...
\textbf{``La porta 5173 \`e gi\`a in uso''}. Significa che un
altro processo di SvelteKit \`e gi\`a partito (forse da una
sessione precedente non chiusa). Su Linux/macOS:
\texttt{./webui/stop.sh} ferma tutto pulitamente. Su Windows:
chiudi la finestra cmd lasciata aperta o termina il processo
\texttt{node.exe} dal Task Manager.

\textbf{``Il browser si apre ma vedo solo una pagina bianca''}.
Apri la console del browser (\texttt{F12}): tipicamente il
problema \`e che il backend non risponde. Verifica
\texttt{http://localhost:8000/api/health} nel browser: deve
restituire un piccolo JSON \texttt{\{"status":"ok"\}}.

\textbf{``Pipeline completata ma orario vuoto''}. Controlla
sul Dashboard che il numero di lezioni sia positivo. Se \`e
zero, probabilmente la pipeline \`e fallita silenziosamente:
guarda i messaggi rossi nel pannello di log (Phase B).

\textbf{``Cambio matrice indisponibilit\`a ma non vedo
effetto''}. Le modifiche entrano in gioco al successivo lancio
di pipeline. Una matrice editata non muove lezioni gi\`a
piazzate: rilancia da \emph{Workflow}.
\end{erroricomunibox}
