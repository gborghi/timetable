\chapter{Terminologia didattica}
...
Le 9 granularit\`a di pricing in $\pi$Tantum, dalla pi\`u
grossolana alla pi\`u fine, sono:

\begin{description}
\item[teacher.] Una colonna per docente. Massimo riuso, minor
  overhead di pricing, ma ogni colonna deve coprire
  \emph{tutta} la settimana del docente. \`E il default.
\item[teacher-day.] Una colonna per (docente, giorno). Ogni
  giornata si pricing\=a separatamente; reagisce meglio a
  vincoli locali (sostituzioni una-tantum).
\item[teacher-class.] Una colonna per (docente, classe).
  Risolve la frammentazione quando un docente lavora in
  classi molto diverse.
\item[teacher-class-subject.] Una colonna per cattedra
  (docente, classe, materia). Massima granularit\`a sui
  pattern di cattedra; ottima per vincoli che riguardano
  ``lezioni di matematica della 1A''.
\item[teacher-subject.] Una colonna per (docente, materia).
  Compromesso fra teacher e teacher-class-subject.
\item[class.] Una colonna per classe. Cambia il punto di
  vista: ogni classe \`e un'entit\`a, non il docente.
\item[class-day.] Una colonna per (classe, giorno). Utile
  per pattern di carico orario per la classe.
\item[day.] Una colonna per giorno: tutti i docenti, tutte
  le classi, una giornata. Pi\`u onerosa ma cattura
  globalmente i vincoli intergiornalieri.
\item[curriculum.] Una colonna per indirizzo. Le decisioni
  vivono al livello di \emph{tutte le classi dello stesso
  indirizzo}, comoda per istituti omogenei.
\end{description}

\sidenote{In codice queste etichette sono il parametro
\texttt{granularity=} di
\texttt{run\_column\_generation()} nel file
\texttt{engine/column\_generation.py}, e per ogni
granularit\`a esiste una funzione di enumerazione
\texttt{\_enumerate\_pricing\_keys()}.}
