...
Se la lezione e\` bloccata nel suo slot (il \emph{pin} impostato da
\texttt{/schedule}), lo spostamento viene rifiutato con
\texttt{needs\_unlock} e la UI chiede conferma. Confermando, la lezione
si sposta e resta \textbf{sbloccata}: il pin nominava quell'ora, ed e\`
l'ora che e\` cambiata. Cambiare la sola aula, a giorno e ora
invariati, non tocca il pin e non chiede nulla.
...
\item \textbf{Modulo supplenze}: quando il sistema cerca un
  candidato per coprire un'assenza, l'ordinamento primario
  \`e disposizione $>$ buco $>$ potenziamento $>$ carico
  residuo; \texttt{graduatoria\_score} resta un
  tie-break secondario. I filtri del modal (stessa materia,
  materie del collega, \ldots) restringono la rosa prima
  dell'ordinamento.
