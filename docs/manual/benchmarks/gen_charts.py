...
def main():
    ...
    args = ap.parse_args()
    rows = load(args.csv)
    print(f"[gen-charts] read {len(rows)} rows from {args.csv}")
    if not rows:
    ...
    with open(args.out, "w", encoding="utf-8") as fh:
        ...
        rows.sort(key=lambda r: (PROFILES.index(r["profile"])
                                  if r["profile"] in PROFILES else 99,
                                  r["family"], r["technique"]))
        emit_summary_table(rows, fh)
        emit_family_avg_table(rows, fh)
        emit_speedup_vs_baseline(rows, fh)
