...
\item For each red cell, click on the cell. A two-column modal
  ...
  \emph{available teachers} on the right. Official disposizione
  (standby) teachers sit at the top (badge \textbf{DISP}), then
  teachers with a hole hour (badge \textbf{BUCO}, emphasised),
  then potenziamento, then the merely free.
\item To pick a colleague of the same subject, use the
  \emph{Stessa materia} (same lesson subject) or \emph{Materie
  del collega} (same subjects as the absent teacher) chips,
  plus the name search if needed. Disposizione placement is
  subject-agnostic: the filters only choose whom to show. The
  \textbf{MAT} badge marks teachers who teach the uncovered
  lesson's subject.
...
\paragraph{Before the first time.} Set each teacher's
disposizione quota on their Teachers card and, from the
\emph{Ore di disposizione} button on the same tab, the school
cap and who is eligible. \emph{Salva e piazza} fills the
high-priority slots (Saturday, first hours). Hours can then
be moved by hand from the Timetable tab.
...
nine uncovered cells. The system puts disposizione teachers and
those with a hole hour at the top; the \emph{Stessa materia}
chip narrows the list to mathematics / history / English
colleagues. The coverage closes in twelve minutes.
\end{esempiobox}
