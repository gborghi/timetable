...
def create_school_classes_with_curriculum(curricula, curriculaclasses={'Scientifico':3, 'ScienzeApplicate':5, 'ScienzeUmane':6, 'EconomicoSociale_FRA':1, 'EconomicoSociale_SPA':1, 'EconomicoSociale_TED':1, 'Linguistico_FRA_TED':1, 'Linguistico_FRA_SPA':2}):
        ...
        while totore>0:
            ...
            while True:
                ...
                if teacher_name not in allnames:
                    ...
                    break
            maxhours=random.choices([18 for j in range(20)]+[16,12,10,16,15])[0]
...
def createmodelvariables(allclasses, allteachers, cconcorsopersubject):
    ...
    for isubject, ihours in iclass.subjects.items():
      for teacher in allteachers:
        if teacher.subject_group.name in cconcorsopersubject[isubject]:
          ...
          #teacher.cattedra[isubject].setdefault({iclass.name: (ihours, a)})
...
def enforcemaxhoursperteacher(allteachers, allclasses, docentedictclasse, docentedictcurriculum):
    ...
    for teacher in allteachers:
        ...
        model.Add(mismatch==floor(teacher.max_hours)-teacher.actual_hours)
...
if __name__=="__main__":
    ...
    createmodelvariables(allclasses, allteachers, cconcorsopersubject)
    ...
    #equalscores=enforce_equal_scores(scoredict, allteachers)
    #model.Minimize((-20*cattedracompleta+20*fewclasses-3*manycurricula)+10*scoremismatch)
    ...
    for teacher, diction in thecattedre.items():
       thisteacher=list(filter(lambda x: x.name==teacher, allteachers))[0]
